...
COCO_NO_CDB_BASE_MODELS=( outputs/wetectron_coco14/weak_1 \
                          outputs/wetectron_coco14/weak_2 \
                          outputs/wetectron_coco14/weak_3 )
COCO_CDB_BASE_MODELS=(  )

TRAIN_INFERENCE_SCRIPT="inference_scripts/run_inference_coco14_train.sh"
TEST_INFERENCE_SCRIPT="inference_scripts/run_inference_coco14_val.sh"
...
# This index is used to select the base model in COCO_NO_CDB_BASE_MODELS
# The selected base model is the model with index (ACTIVE_TRIAL_NUMBER-1)%3
# To replicate Figure 3, run with ACTIVE_TRIAL_NUMBER from 1 to 3
...
ACTIVE_SAVE_NAME_PREFIX="coco14"
...
BASE_NUM_ITERATION=1200

# config path
CONFIG_FILE="configs/coco/V_16_coco14_active.yaml"
...
# "True" or "False", default is False on COCO
USE_CDB="False"
...
LAST_CYCLE_PRED_NAME="coco_2014_train_model_final_no_bbox_aug"
...
if [[ "$USE_CDB" == "True" ]]
then
    NUM_BASE_MODELS=${#COCO_CDB_BASE_MODELS[@]}
    MODEL_INDEX=$(((ACTIVE_TRIAL_NUMBER-1)%NUM_BASE_MODELS))
    ACTIVE_SAVE_NAME_PREFIX="$ACTIVE_SAVE_NAME_PREFIX"_CDB
    DB_METHOD="concrete"
    BASE_EXP_NAME=${COCO_CDB_BASE_MODELS[MODEL_INDEX]}
else
    NUM_BASE_MODELS=${#COCO_NO_CDB_BASE_MODELS[@]}
    MODEL_INDEX=$(((ACTIVE_TRIAL_NUMBER-1)%NUM_BASE_MODELS))
    DB_METHOD="none"
    BASE_EXP_NAME=${COCO_NO_CDB_BASE_MODELS[MODEL_INDEX]}
fi
...
do
    ...
    then
        if [[ "$cycle" == 1 ]]
        then
            # There are normally many BiB pairs in the first cycle,
            # setting a higher score threshold will speed up computations 
            SCORE_THRESH=0.5
        else
            SCORE_THRESH=0.1
        fi
    ...
    LAST_CYCLE_PRED_NAME="$(printf "coco_2014_train_model_%07d_no_bbox_aug" $INFERENCE_ITER )"
done
