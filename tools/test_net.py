...
def main():
    ...
    checkpointer = DetectronCheckpointer(cfg, model, save_dir=ckpt_dir)
    ckpt = cfg.MODEL.WEIGHT if args.ckpt is None else args.ckpt
    _ = checkpointer.load(ckpt, use_latest=ckpt is None)
    ...
    if cfg.TEST.CONCAT_DATASETS:
        dataset_names = ['_'.join(cfg.DATASETS.TEST)]
    else:
    ...
    if cfg.OUTPUT_DIR:
        if ckpt is not None:
            ckpt_suffix = '_' + Path(ckpt).stem
        else:
            ckpt_suffix = '_latest'
        for idx, dataset_name in enumerate(dataset_names):
            output_folder = os.path.join(cfg.OUTPUT_DIR, "inference", dataset_name+ckpt_suffix)
            if not cfg.TEST.BBOX_AUG.ENABLED:
                output_folder += '_no_bbox_aug'
            mkdir(output_folder)
            output_folders[idx] = output_folder
    data_loaders_val = make_data_loader(cfg, is_train=False, is_distributed=distributed)
    for output_folder, dataset_name, data_loader_val in zip(output_folders, dataset_names, data_loaders_val):
        logger = setup_logger("wetectron", output_folder, get_rank(), 'test_logs.txt')
        inference(
            model,
            data_loader_val,
            ...
            vis=args.vis, 
            task=args.task,
            run_evaluation=cfg.TEST.RUN_EVALUATION,
        )
        synchronize()
